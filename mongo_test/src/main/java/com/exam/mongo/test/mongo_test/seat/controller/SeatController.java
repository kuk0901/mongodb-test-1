...
public class SeatController {
  ...
  public ResponseEntity<ResponseDto<String>> putSeatOne(@RequestBody SeatDto seatDto) {
    try {
  ...
  public ResponseEntity<ResponseDto<String>> deleteSeatOne(@PathVariable("id") String id) {
    try {
      return ResponseEntity.ok().body(seatService.deleteSeatOne(id));
    } catch (CustomException e) {
      return ResponseEntity.badRequest().body(new ResponseDto<>("", e.getMessage()));
    } catch (Exception e) {
      return ResponseEntity.internalServerError().body(new ResponseDto<>("", "서버 오류가 발생했습니다. 잠시 후 다시 시도해 주세요."));
